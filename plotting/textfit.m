...
%
% Mike Lawrence 2011
% From: https://stackoverflow.com/questions/4395958/avoid-text-overlap-in-matlab-figures
...
old_pos = nan(n,3);
for i=1:n, old_pos(i,:) = get(h(i),'Position'); end

maxtries = 100;
for t=1:maxtries
  ext = nan(n,4);
  for i=1:n, ext(i,:) = get(h(i),'extent'); end
  % Prevent empty labels from messing everything up
  ext(ext==0) = 0.01;
...
% Plot little lines connecting the new positions to the old
for i = 1:length(old_pos)
    if isempty(h(i).String), continue; end
    new_pos = h(i).Position;
    x = [old_pos(i, 1) new_pos(1)];
    y = [old_pos(i, 2) new_pos(2)];
    if norm([diff(x), diff(y)]) > 0.02 % Approximately one letter distance
        if isempty(z)
            lh = line(x, y);
        else
            lh = line(x, y, [z(i) z(i)]);
